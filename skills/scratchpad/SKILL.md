---
name: scratchpad
description: "Capture out-of-scope discoveries, ideas, tasks, and questions into a per-repo SCRATCHPAD.md while coding, then retrieve, archive, or promote them later. Use this skill whenever the user wants to write something down to deal with later instead of now — \"park this\", \"note that\", \"jot this down\", \"add a note\", \"stick it on the list\", \"look at later\", \"I'll lose this otherwise\" — capturing bugs, ideas, tasks, open questions, things to verify, or surprising findings. Route these here even when it looks like a trivial inline note — the point is durable, retrievable capture, not answering in the moment. Also use it to read items back (\"what's open\", \"what did I park\", \"any notes about X\"), tick them done, clean up/archive, or promote a parked item to a GitHub issue, memory, or project. And fire it yourself when, mid-task, you hit something outside the current scope — often unexpected behavior to work around but not fix now — that would otherwise be lost. This is for TRANSIENT, project-local capture; it is NOT the durable memory system (facts you want auto-loaded every session), reminders, meeting notes, PR text, or code comments."
---
